...
function  GameDeposit() {
  ...
  useEffect(() => {
    ...
    fetch('https://cute-puce-xerus.cyclic.app/api/statistics') // Assuming your backend is running on the same host/port
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://cute-puce-xerus.cyclic.app/api/deposit/history?page=${currentPage}`
        );
  ...
  const handleApprove = async (id, amount) => {
    ...
    try {
      const response = await axios.put(`https://cute-puce-xerus.cyclic.app/api/approve/${id}`, { amount });
  ...
  const deleteDeposit = async (id) => {
    try {
      const response = await axios.delete(
        `https://cute-puce-xerus.cyclic.app/api/gameDeposit/delete/${id}`
      );
