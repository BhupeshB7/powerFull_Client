...
const ContactInfo = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://mlm-production.up.railway.app/api/contactInfo').then((response) => setUserDetails(response.data));
  }, []);
  ...
  const handleSave = () => {
    ...
    axios[requestMethod]('https://mlm-production.up.railway.app/api/contactInfo', userDetails).then(() => {
      setIsEditMode(false);
    });
  };
