...
const ContactInfo = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://mlm-psi.vercel.app/api/contactInfo').then((response) => setUserDetails(response.data));
  }, []);
  ...
  const handleSave = () => {
    ...
    axios[requestMethod]('https://mlm-psi.vercel.app/api/contactInfo', userDetails).then(() => {
      setIsEditMode(false);
    });
  };
