...
const ColorPredictGame = () => {
  ...
  const fetchSessionInfo = async () => {
    try {
      const response = await axios.get(
        "https://mlm-psi.vercel.app/three/getLatestSession"
      );
  ...
  const fetchTimer = async () => {
    try {
      const response = await axios.get(
        `https://mlm-psi.vercel.app/three/api/user/getTimer/${sessionInfo.sessionId}`
      );
  ...
  if(remainingTime===2){
    try {
      const response = axios.post('https://mlm-psi.vercel.app/three/MinuteuserResult',{
    ...
    if (betAmount < 1) {
      ...
      try {
        const response = await axios.post(
          "https://mlm-psi.vercel.app/three/oneMinuteHistory",
          {
            userId: data.userId,
