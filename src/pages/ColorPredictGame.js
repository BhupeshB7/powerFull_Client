...
const ColorPredictGame = () => {
  ...
  const fetchSessionInfo = async () => {
    try {
      const response = await axios.get(
        "https://cute-puce-xerus.cyclic.app/three/getLatestSession"
      );
  ...
  const fetchTimer = async () => {
    try {
      const response = await axios.get(
        `https://cute-puce-xerus.cyclic.app/three/api/user/getTimer/${sessionInfo.sessionId}`
      );
  ...
  if(remainingTime===2){
    try {
      const response = axios.post('https://cute-puce-xerus.cyclic.app/three/MinuteuserResult',{
    ...
    if (betAmount < 1) {
      ...
      try {
        const response = await axios.post(
          "https://cute-puce-xerus.cyclic.app/three/oneMinuteHistory",
          {
            userId: data.userId,
